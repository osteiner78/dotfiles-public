...
local function update_spaces()
  ...
  -- Get all workspaces and their window counts in one call
  sbar.exec([[aerospace list-workspaces --all | while read ws; do count=$(aerospace list-windows --workspace "$ws" | grep -c .); echo "$ws $count"; done]], function(output)
    for line in output:gmatch("[^\r\n]+") do
      local ws, count = line:match("^(%S+)%s+(%d+)$")
      if ws and tonumber(count) and tonumber(count) > 0 then
        local space = sbar.add("item", "space." .. ws, {
          ...
          padding_left = ws == "1" and 0 or 4,
        })
        created_spaces[ws] = space
        add_windows(space, ws)
